...
import { ExternalUser, LinkedLsaUser, PrisonUser } from '../interfaces/user'

const querystring = require('querystring')
const contextProperties = require('../contextProperties')

const processPageResponse = (context: Context) => (response: Response) => {
...
export interface ManageUsersApi {
  ...
  externalUserRoles: (context: Context, userId: string) => Promise<Response>
  externalUserAddRoles: (context: Context, { userId, roles }: { userId: string; roles: string[] }) => Promise<Response>
...
export const manageUsersApiFactory = (client: OAuthEnabledClient): ManageUsersApi => {
  ...
  const put = (context: Context, path: string, body?: string | object): Promise<Response> =>
    client.put(context, path, body).then((response) => response.body)
  const del = (context: Context, path: string): Promise<Response> =>
  ...
  const createRole = (context: Context, role: Role): Promise<Response> => post(context, '/roles', role)
  const getRoles = (context: Context, { adminTypes }: { adminTypes: string }): Promise<Response> =>
  ...
  const getRoleDetails = (context: Context, roleCode: string): Promise<Response> => get(context, `/roles/${roleCode}`)
  const changeRoleName = (context: Context, roleCode: string, roleName: string): Promise<Response> =>
    client.put(context, `/roles/${roleCode}`, roleName)
  const changeRoleDescription = (context: Context, role: string, roleDescription: string): Promise<Response> =>
    put(context, `/roles/${role}/description`, roleDescription)
  const changeRoleAdminType = (context: Context, role: string, adminType: string): Promise<Response> =>
    put(context, `/roles/${role}/admintype`, adminType)

  const createGroup = (context: Context, group: Group): Promise<Response> => post(context, '/groups', group)
  const createExternalUser = (context: Context, user: ExternalUser): Promise<Response> =>
    post(context, `/externalusers/create`, user)
  const groupDetails = (context: Context, { group }: { group: string }): Promise<Response> =>
    get(context, `/groups/${group}`)
  const changeGroupName = (context: Context, group: string, groupName: string): Promise<Response> =>
    put(context, `/groups/${group}`, groupName)
  const deleteGroup = (context: Context, group: string): Promise<Response> => del(context, `/groups/${group}`)

  const createChildGroup = (context: Context, group: Group): Promise<Response> => post(context, '/groups/child', group)
  const childGroupDetails = (context: Context, { group }: { group: string }): Promise<Response> =>
    get(context, `/groups/child/${group}`)
  const changeChildGroupName = (context: Context, group: string, groupName: string): Promise<Response> =>
  ...
  const getAllEmailDomains = (context: Context): Promise<Response> => {
    return client.get(context, `/email-domains`).then(processPageResponse(context))
  }
  const createEmailDomain = (context: Context, domain: string): Promise<Response> =>
  ...
  const currentUser = (context: Context): Promise<Response> => get(context, '/users/me')
  const currentRoles = (context: Context): Promise<Response> => get(context, '/users/me/roles')
  const getUserEmail = async (context: Context, { username }: { username: string }): Promise<Response | {}> => {
    try {
  ...
  const externalUserRoles = (context: Context, userId: string): Promise<Response> =>
  ...
  const assignableRoles = (context: Context, { userId }: { userId: string }): Promise<Response> =>
    get(context, `/externalusers/${userId}/assignable-roles`)
  const searchableRoles = (context: Context): Promise<Response> => get(context, '/externalusers/me/searchable-roles')
  const assignableGroups = (context: Context): Promise<Response> => get(context, '/externalusers/me/assignable-groups')
  const userGroups = (context: Context, { userId }: { userId: string }): Promise<Response> =>
  ...
  const getUser = (context: Context, { userId }: { userId: string }): Promise<Response> =>
    get(context, `/externalusers/id/${userId}`)
  const amendUserEmail = (context: Context, userId: string, email: string): Promise<Response> =>
  ...
  const searchUserByUserName = (context: Context, username: string): Promise<Response> =>
    get(context, `/prisonusers/${username}`)
  const createUser = (context: Context, user: PrisonUser): Promise<Response> => post(context, '/prisonusers', user)
  const createLinkedCentralAdminUser = (context: Context, user: LinkedLsaUser): Promise<Response> =>
    post(context, '/linkedprisonusers/admin', user)
  const createLinkedLsaUser = (context: Context, user: LinkedLsaUser): Promise<Response> =>
    post(context, '/linkedprisonusers/lsa', user)
  const createLinkedGeneralUser = (context: Context, user: LinkedLsaUser): Promise<Response> =>
    post(context, '/linkedprisonusers/general', user)
  const changeDpsEmail = (context: Context, username: string, email: string): Promise<Response> =>
    post(context, `/prisonusers/${username}/email`, email)
  const syncDpsEmail = (context: Context, username: string): Promise<Response> =>
    post(context, `/prisonusers/${username}/email/sync`)
  const contextUserRoles = (context: Context, username: string): Promise<Response> =>
