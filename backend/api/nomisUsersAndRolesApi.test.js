...
describe('nomis users and roles API tests', () => {
  ...
  describe('createAdminUser', () => {
    const user = {
      user: { username: 'JOE_ADM', email: 'joe@digital.justice.gov.uk', firstName: 'joe', lastName: 'smith' },
    }
    beforeEach(() => {
      client.post = jest.fn().mockReturnValue({
        then: () => {},
      })
      nomisUsersAndRolesApi.createAdminUser(context, user)
    })

    it('should call create nomis admin user endpoint', () => {
      expect(client.post).toBeCalledWith(context, '/users/admin-account', user)
    })
  })

  describe('createGeneralUser', () => {
    const user = {
      user: { username: 'JOE_GEN', email: 'joe@digital.justice.gov.uk', firstName: 'joe', lastName: 'smith' },
    }
    beforeEach(() => {
      client.post = jest.fn().mockReturnValue({
        then: () => {},
      })
      nomisUsersAndRolesApi.createGeneralUser(context, user)
    })

    it('should call create nomis general user endpoint', () => {
      expect(client.post).toBeCalledWith(context, '/users/general-account', user)
    })
  })
  describe('createLocalAdminUser', () => {
    const user = {
      user: { username: 'JOE_LA', email: 'joe@digital.justice.gov.uk', firstName: 'joe', lastName: 'smith' },
    }
    ...
    beforeEach(() => {
      ...
      nomisUsersAndRolesApi.createLocalAdminUser(context, user)
    })

    it('should call create nomis local admin user endpoint', () => {
      expect(client.post).toBeCalledWith(context, '/users/local-admin-account', user)
    })
  })
})
